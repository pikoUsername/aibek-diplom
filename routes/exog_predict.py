...
def exog_predict():
	...
	if request.method == "POST":
		...
		# Файл с будущими значениями экзогенных факторов
		...
		if not os.path.exists(filepath):
			return render_template("exog_predict.html", message=f"Файл {csv_filename} не найден!",
			                       plot_filename=None, data_files=data_files,
			                       model_files=model_files, script_files=script_files)
		if not os.path.exists(model_path):
			return render_template("exog_predict.html", message=f"Модель {model_name} не найдена!",
			                       plot_filename=None, data_files=data_files,
			                       model_files=model_files, script_files=script_files)
		...
		for col in [date_col, target_col]:
			if col not in df.columns:
				return render_template("exog_predict.html", message=f"Столбец {col} отсутствует!",
				                       plot_filename=None, data_files=data_files,
				                       model_files=model_files, script_files=script_files)
		...
		if exog_cols:
			if not exog_future_file:
				return render_template("exog_predict.html",
				                       message="Необходимо загрузить CSV с будущими экзогенными данными!",
				                       plot_filename=None, data_files=data_files,
				                       model_files=model_files, script_files=script_files)
			future_exog = pd.read_csv(exog_future_file)
			missing_future = [col for col in exog_cols if col not in future_exog.columns]
			if missing_future:
				return render_template("exog_predict.html",
				                       message=f"В будущем файле отсутствуют колонки: {missing_future}",
				                       plot_filename=None, data_files=data_files,
				                       model_files=model_files, script_files=script_files)
		...
		try:
			...
			return render_template("exog_predict.html", message=f"Ошибка прогнозирования: {e}",
			                       plot_filename=None, data_files=data_files,
			                       model_files=model_files, script_files=script_files)
		...
		plt.legend()
		plt.title("Прогноз с внешними факторами")
		plot_filename = f"exog_forecast_{csv_filename.split('.')[0]}_{uuid.uuid4().hex[:6]}.png"
		plot_path = os.path.join(current_app.config["PLOT_FOLDER"], plot_filename)
		plt.savefig(plot_path)
		plt.close()

		return render_template("exog_predict.html", message="Прогноз сформирован!",
		                       plot_filename=plot_filename, data_files=data_files,
		                       model_files=model_files, script_files=script_files)

	return render_template("exog_predict.html", message=None,
	                       plot_filename=None, data_files=data_files,
	                       model_files=model_files, script_files=script_files)
