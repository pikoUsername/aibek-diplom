...
def returns_predict():
    ...
    if request.method == "POST":
        ...
        if not os.path.exists(filepath) or not os.path.exists(model_path):
            return render_template("returns_predict.html", message="CSV или модель не найдены.", plot_filename=None, data_files=data_files, model_files=model_files)
        df = pd.read_csv(filepath)
        for col in [date_col, returns_col]:
            if col not in df.columns:
                return render_template("returns_predict.html", message=f"Столбец {col} отсутствует.", plot_filename=None, data_files=data_files, model_files=model_files)
        ...
        try:
            ...
            return render_template("returns_predict.html", message=f"Ошибка прогнозирования: {e}", plot_filename=None, data_files=data_files, model_files=model_files)
        ...
        return render_template("returns_predict.html", message="Прогноз сформирован!", plot_filename=plot_filename, data_files=data_files, model_files=model_files)
